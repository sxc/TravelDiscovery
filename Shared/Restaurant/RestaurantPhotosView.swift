...
struct RestaurantPhotosView: View {
    ...
    init() {
        // this changs every UISegmentedControl in your application
    ...
    var body: some View {
        ...
        ScrollView {
            ...
            Picker(selection: $mode, label: /*@START_MENU_TOKEN@*/Text("Picker")/*@END_MENU_TOKEN@*/, content: {
                Text("Grid").tag("grid")
                Text("List").tag("list")
            ...
            if mode == "grid" {
                LazyVGrid(columns:
                            [
                                GridItem(.adaptive(minimum: proxy.size.width / 3 - 6, maximum: 300), spacing: 0),
            ...
            ForEach(photoUrlStrings, id: \.self) { urlString in
                VStack(alignment: .leading) {
                    ...
                    Text("Description for your post and it goes here, make sue to use a bunch of lines of text otherwise you never know what's going to happen.")
                        .font(.system(size: 14))
                        .padding(.horizontal, 8)
                    
                    
                    Text("Posted on 2/10/21")
                        .font(.system(size: 14))
                        .padding(.horizontal, 8)
